...
```
git clone https://github.com/{본인_아이디}/Daily-Book-Club
ex) git clone https://github.com/myID/Daily-Book-Club
```

```
// clone한 폴더로 이동하는 방법
cd {저장소 아이디}
ex) cd Daily-Book-Club
```
...
- 커밋 메시지를 작성하는 과정에서, `commit convention을 준수`합니다.
...
```
ex) 작업 진행한 myID/Daily-Book-Club myID 브랜치 => pengooseDev/Daily-Book-Club main
