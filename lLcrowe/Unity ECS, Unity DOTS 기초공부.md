...
개발이 별로 안 중요해지고 히스토리(역사)가 중요해지고  
왜 이랬는지 저랫는지 알수없고 어쩔수도 없다.  
꼬여있고 정리할 시간도 없기떄문에 기존의 OOP는 문제가 많다.


## -=Data Oriented Design
CPU와 Ram의 성능이 점점 차이가 많이 벌어졋고  
결국 CPU가 엄청 놀고 있으니 지금의 프로그래밍 패턴을 바꾸어  
성능을 낼수 있는 방법을 찾기 시작. 그것이 DOD.

2010년이후
CPU의 멀티코어 시대로 들어서면서 무어의 법칙이 의미가 없어짐.  
=>무어의 법칙도 깨진지 오래다.  

CPU가 빠르냐 Ram이 빠르냐 의미가 없다.  
그냥 CPU가 많기 떄문에 Ram이 아무리 빨라져봐야  
CPU 10배를 넘어서지는 않을테니까

## -=결론 
CPU의 L1,L2,L3캐시 메모리 이런걸 응용해보자고 하는 것.  
OOP의 문제점을 개선 복잡도가 줄었으면 좋겠다.  
최근에 DOD라는게 주목받는 이유로는 언리얼, 유니티 둘다 DOTS패턴을 쓰기 떄문에
...
2. 병렬화 => 스레드적용하기 좋다는거
3. 모듈화 => 추가수정할떄 편리
4. 테스트 => 모듈화되있으니 테스트도 편함

#### -=단점 (이거 때문에 고통받는중)
1. 프로그램을 짜는 방식을 전환 => 패러다임이 다름
2. OOP식으로 짠코드들과 함께동작 => 통합하기 힘들다

#### -=DOD 원리
OOP를 다 DOP로 바꾸기 힘드니 OOP에서 DOP디자인으로 바꿀려면
1. 메모리를 따로 Packing해서 모으고
2. 코드를 ForLoop로 돌린다.

메모리만 Packing하면 되네.
이게 간단하고 편한데, 메모리를 모으는게 쉽지 않다.  
메모리를 계속 패킹상태로 유지하는게 어렵다.  
언제 사라질지 모르니까.  

```
CPU한테 메모리를 가진상태로 던지기가 어렵다  
ECS에서 가장 중요한게 메모리모델
```

### 크게 두종류로 
```
1. SparseSet(마인크래프트, 오버와치)
2. Archetype(언리얼, 유니티)
```
...
ECS는 만능이 아니다.
대규모게임은 좋지만 로그라이크류 게임은 힘듬.  
이유는 몬스터 300종류, 행동 수백 종류같은 다양성이 문제이다.  
각 종류마다 레고블럭을 설치하면 너무 커지지않느냐.  

어덯게 설계하느냐에 따라 다름.  
십만개가 나왔다고 십만개를 For 돌리지는 않는다.  
내부에 안에 트리거가 있고 프레임워크단에서 자동으로 처리해줌.
...
오히려 어려워보이는데 아직 퍼즐이 안맞음.(읽히지가 않음)

---

오버와치가 나오는객체가 적어도 DOP로 한 이유는  
각각 파츠별로 제작하기에 용이하기 때문에 사용.  
...
클라는 아키타입을 권장  
서버는 스파스셋을 권장//원하는되로 가능하니까

---

# ECS : Unity ECS, Unity DOTS (중급 : 실용편 강좌)

DOTS는 URP에서만 돌아감. BuiltIn은 안돌아감.  
...
---
Root에다 스크립트를 박은후 => 각각 게임오브젝트들을 들고온후  
=> 각각 엔티티를 얻은후
...
그것을 알려줄려고 특정 스트럭쳐를 박아서 데이터를 넘겨줘라  
그러면 DOTS가 그 엔티티를 가지고 제어하겠다  

#### 직접 제어하지못하니까 변환이 필요한것
...
기본기능을 알아야 나머지 편의기능을 알수 있다. 적어도 1년을 공부해야된다.

GUID로 가지고 오던 GameObject 주소를 챙겨오던 최종적으로 Entity로 가줘와야됨

오픈월드를 만들려면 SubScene 한개로는 안된다.  
SubScene은 64k 제한이 있음 64개 세션정도면 최대들고 있을수 있다.  

디폴트가 있으므로 0을키고 123을 킬수 있고 0을 끌려면 123을 꺼야지 끌수 있다. (규칙)  
세션0은 바닥종류.   
...
###### -=개인생각
아 바꾸기는 해야되는데 너무 대작업.  
하지만 내가 보는 시야에서는 앞으로 미래는 DOD가 맞다.  
이제껏 하드웨어 사양때문에 시도하지 못한 것들도 가능하며  
구조를 좀더 편하게 짤수 있다는..데 이건 모르겠고.  
더 많은 오브젝트!! 더 빠른 연산!! 이런것들에 꽂힌다.
